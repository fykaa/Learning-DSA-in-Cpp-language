//Now we will create Stack using Linked Lists :
    //everything is simple, problem arises when pop function comes
...
//IMPOSTING TWO WAYS TO CALL A FUNCTION (the problem basically arrived when we were creating pop function for linked list stack)
    //way 1//CALL BY REFERRENCE(here you'll have to dereference each variable , address each variable)
    //way 2//CALL BY VALUE (here you'll have to use GLOBAL VARIABLE)
...
//We don't need to create a STACK function because STACK consists :
    ...
    //TOP -  but in Linked list form of stack , the HEAD is the TOP as every new element inserted is taking constant(O(1)) TIME to be inserted from head side!, hence TOP is included in Linked list datatype instead of head
...
// //  Way 2 OF IMPLEMENTING POP FUNCTION
// #include<stdio.h>
// #include<stdlib.h>
// struct Node {
//     int data;
